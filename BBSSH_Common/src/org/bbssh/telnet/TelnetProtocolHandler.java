...
/**
 * This is a telnet protocol handler. The handler needs implementations for several methods to handle the telnet options
 * and to be able to read and write the buffer.
 * 
...
public abstract class TelnetProtocolHandler {
	 ...
	 * 
	 * @return the size of the terminal as XYRect
	 */
	protected abstract XYPoint getWindowSize();
	...
	public void sendNAWS() throws IOException {
		XYPoint size = getWindowSize();
		write(new byte[] {
				IAC,
				SB,
				TELOPT_NAWS,
	...
	public void inputfeed(byte[] b, int offset, int len) {
		 ...
		 * 
		 * System.arraycopy( tempbuf, 0, xb, 0, tempbuf.length ); System.arraycopy( b, 0, xb, tempbuf.length, len );
		 * tempbuf = xb;
		 */
		byte[] xb = new byte[tempbuf.length + len];
