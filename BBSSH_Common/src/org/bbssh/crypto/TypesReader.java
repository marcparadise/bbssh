...
public class TypesReader {
	...
	public CryptoInteger readTrimmedCryptoIntegerString() throws IOException {
		int len = readUINT32();

		if ((len + pos) > max)
			throw new IOException("Malformed SSH int/string.");

		// Remove padding/sign byte
		if (len == 257 && arr[pos] == 0) {
			pos++; 
			len --; 
		}
		
		// NOTE - crappy RIM CryptoInteger implemnetation is limited to 256 bytes
		// so it is possible that if we get a 257+ byte value we're going to throw an exception..
		if ((len > 256)) {
			throw new IOException("int length too long for RIM CryptoInteger: " + len + " bytes - use BigInteger instead.");
		}
		CryptoInteger ret = new CryptoInteger(arr, pos, len);
		pos += len;
		return ret;
	...
	public String[] readNameList() throws IOException {
		return Tools.splitString(readString(), ',');
