...
abstract public class SshPacket {
	...
	/**
	 * Retrieves a string element and converts it to an array
	 * 
	 * @return string element as an array, using comma as the delimiter
	 */
	public String[] getStringList() {
		return Tools.splitString(getString(), ',');
	}

	public void putNameList(String[] values) {
		StringBuffer buff = new StringBuffer();
		for (int x = 0; x < values.length; x++) {
			buff.append(values[x]);
			if (x != values.length - 1) {
				buff.append(",");
			}

		}
		putString(buff.toString());
	}
	...
	public CryptoInteger getCryptoInteger() {
		int length = getInt32();
		return new CryptoInteger(getBytes(length));
	}
	public byte getByte() {
	...
	public byte[] getBytes() {
		return getBytes(byteArray.length);
	}
	 ...
	 * 
	 * @param str the string to be added to the packet
	 */
