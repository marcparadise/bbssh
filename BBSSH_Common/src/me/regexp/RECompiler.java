...
/**
 * A regular expression compiler class. This class compiles a pattern string into a regular expression program
 * interpretable by the RE evaluator class. The 'recompile' command line tool uses this compiler to pre-compile regular
 * expressions for use with RE. For a description of the syntax accepted by RECompiler and what you can do with regular
...
public class RECompiler {
	...
	/**
	 * Ensures that n more characters can fit in the program buffer. If n more can't fit, then the size is doubled until
	 * it can.
	 * 
	...
	/**
	 * Match an escape sequence. Handles quoted chars and octal escapes as well as normal escape characters. Always
	 * advances the input stream by the right amount. This code "understands" the subtle difference between an octal
	 * escape and a backref. You can access the type of ESC_CLASS or ESC_COMPLEX or ESC_BACKREF by looking at
	 * pattern[idx - 1].
	 * 
	...
	/**
	 * Absorb an atomic character string. This method is a little tricky because it can un-include the last character of
	 * string if a closure operator follows. This is correct because *+? have higher precedence than concatentation
	...
	/**
	 * Compile an expression with possible parens around it. Paren matching is done at this level so we can tie the
	 * branch tails together.
	 * 
